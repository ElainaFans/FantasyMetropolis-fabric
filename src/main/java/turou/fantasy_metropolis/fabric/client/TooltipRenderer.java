...
public class TooltipRenderer {
    ...
    private static Identifier getSpritePath(String part) {
        return Identifier.fromNamespaceAndPath(FantasyMetropolis.MODID, "tooltip/" + part);
    }

    private static Identifier getBorderSpritePath(String borderPart) {
        return getSpritePath("border_" + borderPart);
    }

    private static void innerBlit(Identifier spriteId, int pX, int pY, int pWidth, int pHeight) {
        // duration maybe negative, we don't render negative width
        if (pWidth > 0)
            guiGraphicsContext.blitSprite(RenderPipelines.GUI_TEXTURED, spriteId, pX, pY, pWidth, pHeight);
    }
    ...
    public static List<ClientTooltipComponent> getComponents() {
        ...
        list.add(4,
                Component.literal(
                        ChatFormatting.BLUE + "+ " + AnimationWorker.marqueeDamage(I18n.get("tooltip.attack.damage"))
                                + " " + I18n.get("tooltip.attack.hint")));
    ...
    public static void renderCharacter(int x, int y) {
        guiGraphicsContext.pose().pushMatrix();
        innerBlit(getSpritePath("character_left"), x - 68, y - 16, 58, 80);
        guiGraphicsContext.pose().popMatrix();
    }
    ...
    public static void renderBackground(int x, int y, int width, int height) {
        ...
        innerBlit(getBorderSpritePath("left_top"), horizonStart, verticalStart, 41, 37);
        innerBlit(getBorderSpritePath("top"), horizonStart + 41, verticalStart + 7, horizonDuration, 4);
        innerBlit(getBorderSpritePath("right_top"), horizonEnd, verticalStart, 40, 37);
        innerBlit(getBorderSpritePath("left_bottom"), horizonStart, verticalEnd, 46, 32);
        innerBlit(getBorderSpritePath("left"), horizonStart + 9, verticalStart + 37, 1, verticalDuration);
        innerBlit(getBorderSpritePath("right"), horizonEnd + 30, verticalStart + 37, 1, verticalDuration);
        innerBlit(getBorderSpritePath("right_bottom"), horizonEnd - 6, verticalEnd, 46, 32);
        innerBlit(getBorderSpritePath("bottom"), horizonStart + 46, verticalEnd + 25, width - 66, 2);
        innerBlit(getBorderSpritePath("stars"), horizonCenter + 7, verticalStart + 8, 12, 3);

        // render for background
        innerBlit(getSpritePath("fancy_bg"), x - 6, y - 3, width + 8, height + 6);
