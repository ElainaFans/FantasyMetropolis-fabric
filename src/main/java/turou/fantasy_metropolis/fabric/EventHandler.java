...
public class EventHandler {
    ...
    public static void registerEvents() {
        AttackBlockCallback.EVENT.register((player, world, hand, pos, direction) -> {
            ...
            } else if (block.equals(Blocks.BEDROCK) && itemStack.has(DataComponents.TOOL)
                    && PlayerUtil.hasSword(player)) {
    ...
    public static void registerClientEvents() {
        ClientTickEvents.END_WORLD_TICK.register(world -> {
            ...
            if (result >= 20)
                AnimationWorker.resetTimer();
        });
    }

    // We use mixin to solve our problems.

    public static void onPlayerHurt(Player player, DamageSource source, float amount,
        ...
        if (PlayerUtil.hasSword(player)) {
            ...
            if (attacker != null && !PlayerUtil.hasSword(attacker))
                attacker.hurt(damageSource, Float.MAX_VALUE);
            // player will get no damage
            cir.setReturnValue(false);
        }
    }

    public static void onLivingEntityHurt(LivingEntity receiver, DamageSource damageSource, float damageAmount,
            CallbackInfo ci) {
        var attacker = damageSource.getEntity();
        if (attacker instanceof Player playerAttacker) {
            if (PlayerUtil.hasSword(playerAttacker)) {
                if (!damageSource.isDirect() || ((Player) attacker).getItemInHand(InteractionHand.MAIN_HAND)
                        .getItem() instanceof ItemSwordWhiter) {
